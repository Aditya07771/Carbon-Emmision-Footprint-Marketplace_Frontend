...
export default function BusinessDashboard() {
  ...
  useEffect(() => {
    async function fetchBusinessStats() {
      try {
        const retirements: any = await api.getAllRetirements();
        
        // Filter by current user's wallet (would need company_wallet in backend)
        const myRetirements = retirements.data?.filter(
          (r: any) => r.company?.wallet_address === walletAddress
        ) || [];
        
        const totalTonnes = myRetirements.reduce((sum: number, r: any) => sum + r.tonnes, 0);
        
        setStats({
          totalRetired: totalTonnes,
          creditsHeld: 0, // Would get from blockchain
  ...
  const renderPanel = () => {
    switch (activePanel) {
      case 'overview':
        return <BusinessOverview stats={stats} />;
      case 'marketplace':
...
function BusinessOverview({ stats }: { stats: any }) {
  return (
    <div>
      <div className="mb-8">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <div className="bg-gradient-to-br from-blue-500/20 to-blue-600/10 border border-blue-500/30 rounded-2xl p-6">
          ...
          <button className="w-full py-3 rounded-xl bg-blue-500 text-white font-semibold hover:bg-blue-600 transition-colors">
            Browse Marketplace
          </button>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber/20 to-amber/10 border border-amber/30 rounded-2xl p-6">
          ...
          <button className="w-full py-3 rounded-xl bg-amber text-forest-dark font-semibold hover:bg-amber/90 transition-colors">
            Retire Now
          </button>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-leaf/10 to-emerald-500/5 border border-leaf/20 rounded-2xl p-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="text-center">
            <p className="text-4xl font-bold text-amber mb-2">{stats.totalRetired}</p>
            <p className="text-white/50 text-sm">Tonnes CO₂ Offset</p>
          </div>
          <div className="text-center">
            <p className="text-4xl font-bold text-blue-400 mb-2">
              {stats.totalRetired > 0 ? Math.round(stats.totalRetired * 4.348).toLocaleString() : 0}
            </p>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <div className="bg-white/5 rounded-xl p-4">
            <p className="text-white/70">{userData?.sustainabilityGoals || 'No sustainability goals set yet.'}</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-white/70 text-sm">Annual Target: 1000 tonnes CO₂</span>
                <span className="text-white/70 text-sm">0% Complete</span>
              </div>
              <div className="w-full bg-white/10 rounded-full h-3">
                <div className="bg-gradient-to-r from-leaf to-emerald-500 h-3 rounded-full" style={{ width: '0%' }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['SDG 13', 'SDG 15', 'SDG 7', 'SDG 12'].map((sdg) => (
              <div key={sdg} className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                <p className="text-white font-semibold">{sdg}</p>
                <p className="text-white/50 text-xs mt-1">Aligned</p>
              </div>
            ))}
          </div>
        </div>
